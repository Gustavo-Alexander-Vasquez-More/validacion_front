...
export default function nuevoLeonVal() {
  ...
  const licenciaNuevoLeon2=allLicencia?.filter(licencia=>licencia.estado_id.nombre === 'Nuevo León')
  const licenciaEncontrada = licenciaNuevoLeon?.find((item) => item.folio === folio);
  const licenciaEncontradaAll = licenciaNuevoLeon2.find((item) => item.folio === folio);
